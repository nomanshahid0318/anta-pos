...
function renderUsers(){const el=$('users-table')||$('u-table');if(el)el.innerHTML=DATA.users.map(u=>`<tr><td>${u.Name}</td><td>${u.StoreName||u.StoreID}</td><td><span class="badge badge-blue">${u.Role}</span></td><td>${u.StandardSalary>0?fmt(u.StandardSalary):'—'}</td><td class="fw7">${u.UserID}</td><td class="fw7" style="font-family:monospace;color:var(--accent2)">${u.EmployeeCode||'—'}</td><td><span class="badge ${u.PosLoginEnabled?'badge-green':'badge-gray'}">${u.PosLoginEnabled?'Enabled':'Off — payroll only'}</span></td><td><span class="badge ${u.Active==='N'?'badge-red':'badge-green'}">${u.Active==='N'?'Inactive':'Active'}</span></td><td><button class="btn btn-ghost btn-sm" onclick="editUser('${u.UserID}')">✏️</button> <button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="toggleUserActive('${u.UserID}')">${u.Active==='N'?'✅ Activate':'🚫 Deactivate'}</button> <button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="deleteUserRow('${u.UserID}')">🗑️ Delete</button></td></tr>`).join('');}
