...
if(Array.isArray(users))DATA.users=users.map(u=>({UserID:u.user_id,StoreID:u.store_id,StoreName:u.store_name,Name:u.name,Role:u.role,Active:u.active?'Y':'N',PosLoginEnabled:u.posLoginEnabled!==false}));
...
function renderUsers(){const el=$('users-table')||$('u-table');if(el)el.innerHTML=DATA.users.map(u=>`<tr><td class="fw7">${u.UserID}</td><td>${u.Name}</td><td>${u.StoreName||u.StoreID}</td><td><span class="badge badge-blue">${u.Role}</span></td><td><span class="badge ${u.PosLoginEnabled?'badge-green':'badge-gray'}">${u.PosLoginEnabled?'Enabled':'Off — payroll only'}</span></td><td><span class="badge ${u.Active==='N'?'badge-red':'badge-green'}">${u.Active==='N'?'Inactive':'Active'}</span></td><td><button class="btn btn-ghost btn-sm" onclick="editUser('${u.UserID}')">✏️</button> <button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="toggleUserActive('${u.UserID}')">${u.Active==='N'?'✅ Activate':'🚫 Deactivate'}</button></td></tr>`).join('');}
let editingUserId=null;
function showAddUser(){editingUserId=null;if($('user-form')){$('user-form').style.display='flex';['u-nm','u-pin'].forEach(id=>{if($(id))$(id).value='';});if($('u-role'))$('u-role').value='cashier';if($('u-pin'))$('u-pin').placeholder='4-digit PIN';if($('u-login-enabled'))$('u-login-enabled').checked=true;toggleUserPinField();const t=document.querySelector('#user-form .modal-title');if(t)t.textContent='➕ Add User';}}
...
async function saveUser(){
  ...
  if(!editingUserId&&loginEnabled&&!pin){toast('PIN required when POS/HO Login is enabled','error');return;}
