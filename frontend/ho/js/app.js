...
function renderUsers(){
  ...
  if(loginEl)loginEl.innerHTML=DATA.users.map(u=>`<tr><td class="fw7">${u.Name}</td><td>${u.UserID}</td><td style="font-family:monospace;color:var(--accent2)">${u.EmployeeCode||'—'}</td><td><span class="badge ${u.PosLoginEnabled?'badge-green':'badge-gray'}">${u.PosLoginEnabled?'Enabled':'Off — payroll only'}</span></td><td><button class="btn btn-ghost btn-sm" onclick="editUser('${u.UserID}')">✏️</button> <button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="toggleUserActive('${u.UserID}')">${u.Active==='N'?'✅ Activate':'🚫 Deactivate'}</button> <button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="deleteUserRow('${u.UserID}')">🗑️ Delete</button></td></tr>`).join('');
}
